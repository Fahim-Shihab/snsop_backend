...
public class BeneficiaryEditConsumer {
    ...
    @RabbitListener(queues = "${kit.rabbit.edit.queue}")
    public void receiveMessage(Message message, BeneficiaryDto.BeneficiaryEditRequest beneficiaryRequest)
    {
        try {
            ...
            try {
                ...
                if (beneficiary == null) {
                    log.info("Beneficiary with application id {} doesn't exists", beneficiaryRequest.getApplicationId());
                    return;
                }
                service.updateBeneficiary(beneficiaryRequest);
                if (requestLog != null) {
                ...
                log.error("===REQUEST EDIT PROBLEM IN DATABASE WITH APPLICATION ID:{}", beneficiaryRequest.getApplicationId());
            }
        }finally {
